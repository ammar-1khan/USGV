...
# ser = serial.Serial('/dev/ttyUSB0', 9600)  # Adjust the port and baud rate according to your setup
        ...
        if len(det):
            for *xyxy, conf, cls in reversed(det):
                ...
                if class_name in detected_objects and conf >= 0.70:  # Check if it's a desired object with high confidence
                    ...
                    cv2.rectangle(img, (int(xyxy[0]*5), int(xyxy[1]*5)), (int(xyxy[2]*5), int(xyxy[3]*5)), (255, 0, 0), 2)
...
def main():
    ...
    # model, stride, names, imgsz = load_yolov5_model(weights_path, device=device)
    # Load YOLO model
    ...
    while True:
        ...
        if tracking:
            ...
            if len(faces) > 0:
                ...
                # print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Face detected in region:",
                      # grid_labels[region_number])
                
                # Print the region name
                #region_name = grid_labels[region_number]
                #print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "Face detected in region:", region_name)
